...
wd_desktop <- "C:\\Users\\kristin.barker\\Documents\\GitHub\\HabitatModelling\\Lab9_cLogit2\\Moose"
wd_laptop <- "C:\\Users\\kjbark3r\\Documents\\GitHub\\HabitatModelling\\Lab9_cLogit2\\Moose"
...
```
#### Note - to get the elevations I used the new `elevatr` package - 
# doing something like this to the raw moose data:

```{r}
require(elevatr)
proj <- "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
moose.daily <- mutate(moose.daily, 
                      elevation = get_elev_point(data.frame(x = lon, y = lat), 
                                                 prj = proj, src = "mapzen", 
                                                 api_key = "mapzen-91tYY4s")@data[,1])
```
#~# did work; ignore error message
...
scan.track.z <- function(time, x, y, z, title = "",...)
{
  ...
  plot(x,y,asp=1, type="o", pch=19, col=rgb(z/max(z),(1-z/(max(z))),0,.5), 
       cex=0.5, ...); MakeLetter(title)
  plot(time,x, type="o", pch=19, col=rgb(0,0,0,.5), xaxt="n", xlab="", 
       cex=0.5, ...); MakeLetter("X")
  plot(time,y, type="o", pch=19, col=rgb(0,0,0,.5), xaxt="n", xlab="", 
       cex=0.5, ...); MakeLetter("Y")
...
```
#~# "dreamy" migrations, sweet plots of movement
#### 2.0 Playing with MODIS

#### An important warning ... 
# the CRAN MODISTools does NOT work because of outdated / 
# unsupported dependencies (see this thread: 
# https://groups.google.com/forum/#!topic/predicts_project/FctDowcYgx0).  You have to install if from GIT-hub: 
